...
suiteDeTestsDeParteII =
  describe "Parte 2: Temperaturas" $ do
    describe "celsiusAFahrenheit" $ do
      it "B-) Pasa una temperatura de celsius a fahrenheit" $ do
        celsiusAFahrenheit 100 `shouldBeEqualUpTo2Decimals` 212
      it "B-) ... incluso si la temperatura es negativa" $ do
        celsiusAFahrenheit (-100) `shouldBeEqualUpTo2Decimals` (-148)

    describe "fahrenheitACelsius" $ do
      it "B-) Pasa una temperatura de fahrenheit a celsius" $ do
        fahrenheitACelsius 212 `shouldBeEqualUpTo2Decimals` 100
      it "B-) ... incluso si la temperatura es negativa" $ do
        celsiusAFahrenheit (-148) `shouldBeEqualUpTo2Decimals` (-100)

    describe "fahrenheitACelsius y celsiusAFahrenheit son inversas" $ do
      it "B-) Convertir un valor en celsius a fahrenheit y luego volver a convertir a celsius retorna el valor original" $ do
        fahrenheitACelsius (celsiusAFahrenheit 100) `shouldBeEqualUpTo2Decimals` 100
      it "B-) Convertir un valor en fahrenheit a celsius y luego volver a convertir a fahrenheit retorna el valor original" $ do
        celsiusAFahrenheit (fahrenheitACelsius 212) `shouldBeEqualUpTo2Decimals` 212
      

    describe "haceFrioCelsius" $ do
      it "B-) Es verdad cuando hacen menos de 8 grados celsius" $ do
        haceFrioCelsius 7 `shouldBe` True
      it "B-) Es verdad cuando hacen exactamente 8 grados celsius" $ do
        haceFrioCelsius 8 `shouldBe` True
      it "B-) Es falso cuando hacen más de 8 grados celsius" $ do
        haceFrioCelsius 9 `shouldBe` False

    describe "haceFrioFahrenheit" $ do
      it "B-) Es verdad cuando hacen menos de 46.4 grados fahrenheit" $ do
        haceFrioFahrenheit 46.3 `shouldBe` True
      it "B-) Es verdad cuando hacen exactamente 46.4 grados fahrenheit" $ do
        haceFrioFahrenheit 46.4 `shouldBe` True
      it "B-) Es falso cuando hacen más de 46.4 grados fahrenheit" $ do
